...
const toggleSelectAll = () => {
    selectedUsers.value = isAllSelected.value
        ? []
...
const applyCrop = () => {
    ...
    if (canvas) {
        ...
        canvas.toBlob(
            ...
            "image/webp",
            0.8,
        ); // 0.8 adalah kompresi (80%), udah sangat optimal dan ringan buat WebP
    }
};
...
const handleBulkDelete = () => {
    ...
    if (
        confirm(
            `🚨 SECURITY WARNING: Bulk delete ${selectedUsers.value.length} accounts?`,
        )
    ) {
        router.post(
            "/users/bulk-delete",
...
<template>
    ...
    >
            ...
            >
                ...
                <div class="flex items-center gap-3">
                    <button
                        ...
                        class="inline-flex items-center gap-2 px-4 py-2.5 bg-rose-50 border border-rose-200 text-rose-700 font-bold text-xs rounded-xl shadow-sm transition"
                    >
                        <svg
                    ...
                    <button
                        ...
                        class="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-md transition"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            class="h-4 w-4"
            ...
            >
                <div class="relative flex-1 max-w-md">
                    ...
                    <input
                        type="text"
                        v-model="searchQuery"
                        placeholder="Search dynamically by name or email..."
                        class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 focus:border-blue-500 rounded-xl text-xs font-semibold focus:outline-none transition"
                    />
                </div>
                <div class="flex items-center gap-3">
                    <label
                    ...
                    <select
                        ...
                        class="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl text-xs font-semibold focus:outline-none cursor-pointer"
                    >
                    ...
                    >
                            ...
                            <tr>
                                <th class="px-6 py-4 w-12 text-center">
                                    <input
                                        ...
                                        class="rounded border-slate-300 text-blue-600 h-4 w-4 transition cursor-pointer"
                                    />
                                </th>
                                <th class="px-6 py-4">
                        ...
                        >
                            <tr
                                ...
                                :class="{
                                    'bg-blue-50/30': selectedUsers.includes(
                                        user.id,
                                    ),
                                }"
                            >
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    <div
                                        class="p-4 bg-white border border-slate-200 rounded-2xl shadow-sm space-y-4"
                                    >
                                        <label
                                            class="block text-xs font-bold text-slate-400 uppercase tracking-wider"
                                            >Profile Avatar (Optional)</label
                                        >

                                        <div class="flex items-center gap-4">
                                            <div
                                                class="h-16 w-16 rounded-full bg-slate-800 border border-slate-700 shadow-sm overflow-hidden flex items-center justify-center text-white text-sm font-black uppercase"
                                            >
                                                <img
                                                    v-if="imagePreview"
                                                    :src="imagePreview"
                                            ...
                                            <div class="flex-1">
                                                <input
                                                    ...
                                                    class="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 file:cursor-pointer transition"
                                                />
                                                <p
                                    ...
                                    >
                                        <div
                                            class="flex items-center justify-between p-3.5 bg-white border border-slate-200/80 rounded-2xl shadow-sm"
                                        >
                                            <div
                                            ...
                                            >
                                                ...
                                                >
                                                <span
                                                    class="text-[11px] font-medium text-slate-400 mt-0.5 truncate"
                                                    >{{
                                                        form.is_locked
                                                            ? "🔴 Locked out."
                                                            : "🟢 Accessible."
                                                ...
                                                <span
                                                    :class="
                                                        form.is_locked
                                                            ...
                                                            : 'translate-x-0'
                                                    "
                                                    class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition duration-200"
                                                ></span>
                                            </button>
                                        </div>
                                        <div
        ...
        >
            ...
            >
                <div
                    class="px-5 py-4 bg-slate-900 text-white flex items-center justify-between"
                >
                    <h3 class="text-xs font-bold uppercase tracking-wider">
                        Adjust & Crop Photo
                    </h3>
                    <span class="text-[10px] text-slate-400 font-medium"
                        >Native Vue Cropper</span
                    >
                </div>

                <div
                    class="p-6 bg-slate-100 flex items-center justify-center max-h-[400px] overflow-hidden"
                >
                    <cropper
                        ref="cropperRef"
                        :src="imageToCropSrc"
                ...
                <div
                    class="px-5 py-4 bg-white border-t border-slate-100 flex items-center justify-end gap-2.5"
                >
                    <button
                        type="button"
                        @click="cancelCrop"
                    ...
                    <button
                        type="button"
                        @click="applyCrop"
                        class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-md transition"
                    >
                        ✓ Apply Crop
                    </button>
                </div>
